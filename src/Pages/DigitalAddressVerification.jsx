...
import mapTrue from "../Images/mapTrue.png"
import mapFalse from "../Images/mapFalse.png"
...
const DigitalAddressVerification = () => {
    ...
    const handleShowPopup = (field) => {
        ...
        switch (field) {
            ...
            case "nearest_landmark":
                ...
                break;
            case "map_screenshot":
                title = "Examples: Map Screenshot";
                examples = [
                    {
                        valid: true,
                        imageUrl: `${mapTrue}`,
                        description: "Correct: A Google Maps screenshot clearly showing a nearby, well-known landmark with labels (e.g., school, hospital, temple)."
                    },
                    {
                        valid: false,
                        imageUrl: `${mapFalse}`,
                        description: "Incorrect: Screenshot without any identifiable labels or landmarks (e.g., plain road, empty area, or zoomed-out map)."
                    }
                ];
                break;
            default:
    ...
    const validate = () => {
        ...
        const fileFields = ['id_proof', 'house_name_main_door', 'building_photo', 'street_photo', 'nearest_landmark','map_screenshot']; // Define dynamic file fields
                    ...
                    <>
                        ...
                        <div className="border md:w-7/12 bg-white rounded-md m-auto p-4 ">
                            ...
                            <div className="md:grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className=" my-3 form-group">
                                    <label htmlFor="map_screenshot" className="block text-sm font-medium text-gray-700"> Upload Map Screenshot<span className='text-red-500'>* </span>
                                        <button type="button" onClick={() => handleShowPopup("map_screenshot")} className="text-blue-600 text- underline ">
                                            Show Examples
                                        </button></label>
                                    <input type="file" className="mt-1 block w-full border-gray-300 rounded-md border p-2" id="map_screenshot" name="map_screenshot" onChange={(e) => handleFileChange('map_screenshot', e)}
                                        accept=".jpg,.jpeg,.png,.pdf,.docx,.xlsx" multiple />
                                    {errors.map_screenshot && (
                                        <p className="text-sm text-red-500 mt-1">{errors.map_screenshot}</p>
                                    )}
                                </div>
