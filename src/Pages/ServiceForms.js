...
const ServiceForms = () => {
    ...
    const handleAddInput = (rowIndex) => {
        ...
        if (currentInputs < totalHeaders) {
            ...
            const newInput = {
                type: 'text',  // or 'file', depending on your condition
                name: labelName, // Name derived from label
                options: [],     // For select/dropdown (empty here)
                multiple: false, // Only for file input
            };
    
            row.inputs.push(newInput); // Add only one input
            setFormData({ ...formData, rows: newRows });
        } else {
